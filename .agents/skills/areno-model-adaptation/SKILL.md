...
4. **Training:** run forward/backward and verify finite nonzero gradients. Compare rollout and fixed-token train logprobs.
...
Provide config mapping, checkpoint coverage, reference comparison, coherent decode, backward/gradient result, save/reload comparison, requested topology, and CUDA graph evidence. A successful weight load is only phase 2.
