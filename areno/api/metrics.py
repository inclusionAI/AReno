...
def record_training_stats(writer, stats, step, train_res, train_batch, timings: dict[str, float] | None = None):
    ...
    # `reward/<name>_invalid_count`; because of the `train/` prefix below, each
    # component is written as the scalar `train/reward/<name>_mean` (and
    # `train/reward/<name>_invalid_count`) on TensorBoard, with no dedicated
    # writer branch.
