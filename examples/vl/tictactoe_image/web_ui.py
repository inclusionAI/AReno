        ...
        messages=[
            {"role": "system", "content": _system_prompt(server.agent_player)},
            {
                ...
                "content": [
                    {
                        "type": "image_url",
                        "image_url": {"url": f"data:image/png;base64,{_board_image_base64(server.board)}"},
                    },
                    {"type": "text", "text": _turn_prompt(server.board, server.agent_player)},
                ],
            },
        ],
...
def _board_image_base64(board: game.Board) -> str:
    buffer = io.BytesIO()
    dataset_generator.render_board(board).save(buffer, format="PNG")
