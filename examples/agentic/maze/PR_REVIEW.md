...
6. **经验总结**：在 POMDP 中，状态不只是位置，还包括 agent 携带的物品。搜索算法的状态空间必须覆盖所有影响可达性的变量
...
6. **经验总结**：奖励设计在 RL 中的重要性不亚于模型本身。应在写 reward 函数之前就分析稀疏性，而非等训练结果才发现
...
6. **经验总结**：小迷宫的唯一变体数量是有限且有上界的，生成器需要 graceful degradation 而非 crash
