...
Key components:
  - Board type (list[list[int]]) and constants (DIRECTIONS, SIZE, etc.)
  - move(): applies a swipe, returns (new_board, score, valid, terminal)
  - normalize_board(): parses board from list/JSON string
  - empty_count(): counts empty cells for reward computation
  - parse_action(): extracts direction keyword from free-form text
  - render_board(): produces text representation for LLM prompts
  - SYSTEM_PROMPT / MOVE_TOOL: prompt template and tool-call schema
"""
