...
def _format_record(raw: dict, index: int) -> dict:
    ...
    # Graduate max_turns by difficulty (curriculum). When the curriculum env
    # switch is off, fall back to the flat DEFAULT_MAX_TURNS / record value.
    if _curriculum_enabled() and "max_turns" not in raw:
        max_turns = DIFFICULTY_MAX_TURNS.get(difficulty, DEFAULT_MAX_TURNS)
    else:
