...
def _assistant_message(response: Any) -> dict[str, Any]:
    ...
    # we can re-feed it into messages and mutate it safely.
...
def _execute_tool(env: sudoku.SudokuEnv, call: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        # Illegal input (bad coord, non-empty cell, undo at start, terminal).
        # Counts as a used action only for place/undo inside the env; here we
        # surface the message without leaking the solution.
