...
class TestWordValidation:
    ...
    def test_normalize_word_invalid_length(self):
        ...
        with pytest.raises(ValueError, match="must be 5 letters"):
            game.normalize_word("helloworld")
...
class TestGuessChecking:
    ...
    def test_repeated_letters_one_match(self):
        ...
        result = game.check_guess(guess, target)
        # Target: w-o-r-l-d (one 'l' at position 3)
        # Guess:  l-l-a-m-a (two 'l's at positions 0 and 1)
        # First 'l' at position 0: PRESENT (in target but wrong position)
        # Second 'l' at position 1: ABSENT (target's only 'l' already counted)
    ...
    def test_repeated_letters_exact_and_present(self):
        ...
        result = game.check_guess(guess, target)
        # Target: a-b-i-d-e (one 'i' at position 2)
        # Guess:  i-c-i-n-g (two 'i's at positions 0 and 2)
        # Position 2: 'i' is EXACT (matches target at position 2)
        # Position 0: 'i' is ABSENT (target's only 'i' already matched at position 2)
...
class TestScoring:
    ...
    def test_score_won(self):
        """Test scoring for won game (capped at 1.0)."""
        g = game.create_new_game("hello")
        g = game.apply_guess(g, "hello")
...
class TestRewardBounded:
    ...
    def test_reward_no_tool_is_worst(self):
        ...
        # Use a guess that is in the word list (valid) but not the target, so
        # the reward is either REWARD_NO_MATCH or higher, but never -1.0.
        tool_call = reward.reward_fn(_FakeRecord(target="hello", guess_word="world"))
...
class TestDifferentWordLengths:
    ...
    def test_variable_length_check_guess(self):
        ...
        assert result[4] == game.LetterStatus.ABSENT  # e (not in target)
        assert result[5] == game.LetterStatus.PRESENT  # t (in target at position 4)
...
class TestBoundaryValues:
    """Test boundary values and edge cases (Issue #189)."""

    def test_win_on_last_guess(self):
        """Test game can be won on the 6th (last) guess."""
        g = game.create_new_game("hello")
        for guess in ["world", "earth", "bread", "fruit", "water"]:
            g = game.apply_guess(g, guess)
        assert g["state"] == game.GameState.IN_PROGRESS
        assert len(g["guesses"]) == 5
    ...
    def test_empty_results_compute_stats(self):
        """Test compute_stats with empty results returns zeroes."""
        stats = game.compute_stats([])
        assert stats["overall_solve_rate"] == 0.0
        assert stats["overall_avg_guesses"] == 0.0
        assert stats["by_word_length"] == {}
    ...
    def test_invalid_guess_not_in_word_list(self):
        """Test applying a guess not in WORD_LIST raises ValueError."""
        g = game.create_new_game("hello")
        with pytest.raises(ValueError, match="Invalid word"):
            game.apply_guess(g, "zzzzz")


class TestIntegration:
    """Integration-style tests crossing module boundaries (Issue #189)."""

    def test_dataset_loader_format(self):
        """Test dataset_loader validates path before expensive operations."""
        from dataset_loader import load_training_dataset, DatasetValidationError

        with pytest.raises(DatasetValidationError, match="Path does not exist"):
            load_training_dataset("/tmp/nonexistent.jsonl")

    def test_generated_records_valid_in_game(self):
        """Test records generated by dataset_generator work with game logic."""
        from dataset_generator import generate_records
        ...
        for record in records:
            ...
            assert g["state"] == game.GameState.IN_PROGRESS
            # A correct guess should win
            g = game.apply_guess(g, target)
            assert g["state"] == game.GameState.WON
