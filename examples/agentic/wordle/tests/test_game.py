...
class TestIntegration:
    ...
    def test_dataset_loader_format(self):
        """Test dataset_loader produces valid prompt records."""
        from dataset_loader import load_training_dataset

        records = load_training_dataset("/tmp/nonexistent.jsonl")
        assert len(records) > 0
        for record in records:
            assert "id" in record
            assert "prompt" in record
            assert "target" in record
            assert "game" in record
            assert record["target"] == record["game"]["target"]
