...
from areno.dashboard.flow import provider
...
def test_discovery_cache_expires_without_modal_credentials(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("areno.dashboard.flow.server.latest_image", latest)
    monkeypatch.setattr("areno.dashboard.flow.server.time.time", lambda: 100)
    app = Application(tmp_path)
    assert app.get("/api/images/latest", {})["tag"] == "v1.2.3"
    app.get("/api/images/latest", {})
    assert len(calls) == 1
    monkeypatch.setattr("areno.dashboard.flow.server.time.time", lambda: 161)
    app.get("/api/images/latest", {})
    assert len(calls) == 2
