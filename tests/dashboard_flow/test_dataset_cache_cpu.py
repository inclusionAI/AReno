...
from areno.dashboard.flow.dataset_cache import cache_key, download_repository, repository_files
from areno.dashboard.flow.datasets import resolve_request, save_dataset
from areno.dashboard.flow.samples import dataset_sample
...
def test_modelscope_complete_shards_are_uploaded_with_hf_model(tmp_path, monkeypatch):
    ...
    content = b'{"answer":42}\n' * 4
    monkeypatch.setattr(
        "areno.dashboard.flow.samples.request_json",
        lambda *args: {
            "Code": 200,
    ...
    monkeypatch.setattr("areno.dashboard.flow.dataset_cache.urllib.request.urlopen", download)
...
def test_hf_selects_all_requested_split_shards_and_rejects_partial(monkeypatch):
    ...
    monkeypatch.setattr("areno.dashboard.flow.samples.request_json", lambda *args: result)
