...
from areno.dashboard.flow import remote
...
def test_original_model_snapshots_use_volume_cache_and_reuse_refs(monkeypatch, tmp_path):
    ...
    from areno.dashboard.flow.remote import cache_model_refs
...
def test_provider_preparation_build_and_gpu_reservation(kind):
    ...
    from areno.dashboard.flow.provider import ModalProvider
...
def test_failed_stream_is_reattached_and_recovers_readiness(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("areno.dashboard.flow.controller.time.sleep", lambda _: real_sleep(0.001))
