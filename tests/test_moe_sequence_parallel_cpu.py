...
def test_bailing_v3_decoder_checkpoints_attention_and_dense_mlp(monkeypatch):
    ...
    def checkpoint(function, states, *args, train_meta=None, infer_meta=None, **kwargs):
        del train_meta, infer_meta, kwargs
...
def test_bailing_v3_decoder_keeps_sparse_router_outside_recompute(monkeypatch):
    ...
    class SparseMlp:
        def __call__(self, states, num_padding_tokens):
            sparse_calls.append(num_padding_tokens)
            return states * 3
    ...
    assert checkpointed == ["attention_block"]
